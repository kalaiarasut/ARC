...
const DEFAULT_BATCH_SIZE = parseIntOr(
  Deno.env.get("REPORT_TRANSLATION_BATCH_SIZE"),
  10,
);
const MAX_BATCH_SIZE = parseIntOr(
  Deno.env.get("REPORT_TRANSLATION_MAX_BATCH_SIZE"),
  40,
);
const DEFAULT_CONCURRENCY = parseIntOr(
  Deno.env.get("REPORT_TRANSLATION_CONCURRENCY"),
  3,
);
const MAX_CONCURRENCY = parseIntOr(
  Deno.env.get("REPORT_TRANSLATION_MAX_CONCURRENCY"),
  8,
);
...
Deno.serve(async (request: Request) => {
  ...
  await runWithConcurrency(eligible, concurrency, async (report: any) => {
